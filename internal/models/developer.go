...
func GetDeveloperPage(userID string) (*DeveloperPage, error) {
	p := &DeveloperPage{}
	var linksJSON, featuredJSON string
	err := storage.DB.QueryRow(
		`SELECT user_id, display_name, banner_url, theme_color, COALESCE(theme_preset,'steam-dark'), custom_css, links, about,
		        COALESCE(font_heading,''), COALESCE(font_body,''), COALESCE(featured_games,'[]'), created_at, updated_at
		...
		&p.FontHeading, &p.FontBody, &featuredJSON, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func UpsertDeveloperPage(userID, displayName, bannerURL, themeColor, themePreset, about, fontHeading, fontBody string, links []DeveloperLink, featuredGames []string) error {
	linksJSON, _ := json.Marshal(links)
	featuredJSON, _ := json.Marshal(featuredGames)
	_, err := storage.DB.Exec(
		`INSERT INTO developer_pages (user_id, display_name, banner_url, theme_color, theme_preset, links, about, font_heading, font_body, featured_games)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		...
		userID, displayName, bannerURL, themeColor, themePreset, string(linksJSON), about, fontHeading, fontBody, string(featuredJSON),
	)
	return err
}
