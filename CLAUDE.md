...
internal/
  ...
  lobby/                   # In-memory multiplayer lobby hub (issue #29): lobbies, JSON protocol,
                           #   message relay, per-game online counts. No DB — ephemeral by design.
...
- **CSP**: Per-request 16-byte nonce generated in `server.go` NoRoute handler, injected into `<style>`/`<script>` tags. `script-src-attr 'none'` — the SPA has **no inline `on*=` handlers**; all events go through delegated listeners (see Frontend below). `style-src-attr 'unsafe-inline'` is still allowed (inline `style=` attributes are used heavily). GoatCounter URL extends `script-src` / `connect-src` when configured.
...
- **Events (no inline handlers)**: CSP blocks inline `on*=` (`script-src-attr 'none'`). Emit `' + act('fnName', ...args) + '` for clicks or `' + onEv('change'|'input'|'focus', 'fnName', ...args) + '` for other events instead of `onclick="fnName(...)"`. These emit `data-act`/`data-<event>` + JSON-encoded, HTML-escaped `data-*-args`; delegated listeners dispatch to the global `window[fnName]` with `this` bound to the element and args spread in. `closest()` resolves the innermost target (no `stopPropagation` needed). Hover/focus visual effects go in CSS, not handlers.
- **Game serving**: Files at `/play/<id>/*filepath` via iframe. ZIP uploads auto-extracted, entry HTML detected. WebGPU works natively. `gzip` middleware excludes `/play/` to keep Range support intact (and `/ws` — the upgrade needs the raw `http.Hijacker`).
- **Multiplayer lobbies (#29)**: `GET /ws` (root-mounted, not under `/api` — exempt from the OpenAPI drift test) upgrades to WebSocket via `coder/websocket`; `Accept`'s default same-host Origin check is the CSRF equivalent (WS handshakes are GETs and bypass `CSRFProtect`). `internal/lobby` holds the in-memory hub (`lobby.Default`): create/join by 6-char code, ready check, host-only start, opaque message relay (broadcast or per-player `to`). Lobby dies with its host; auto-leave on re-create/join; limits are consts in `hub.go` (8 players, 8 KiB frames, 30 msg/s, 2h idle TTL, 4 conns/user, 500 lobbies). Games opt in via the `multiplayer` column; the SPA bridges lobby⇄iframe with postMessage (protocol in `docs/DEVELOPER.md`). Cleanup goroutine started in `main.go` (`lobby.Default.StartCleanup`).
